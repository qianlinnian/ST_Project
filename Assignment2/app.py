...
if page == "1. Requirement Input":
    ...
    st.caption("Mock data can be used before D/E provide the final TodoList requirements.")
    st.data_editor(st.session_state.requirements, num_rows="dynamic", use_container_width=True)

if page == "2. Structuring & Risk":
    st.subheader("Requirement Structuring and Risk Analysis")
    elapsed, structured = measure_time(structure_requirements, st.session_state.requirements)
...
if page == "4. Test Cases":
    ...
    coverage_items = identify_coverage_items(structured, risks)
    strategies = select_strategies(coverage_items)
    elapsed, test_cases = measure_time(generate_test_cases, structured, coverage_items, strategies)
...
if page == "5. AI Review":
    ...
    if not is_llm_enabled():
        st.warning("LLM API is not configured. Copy .env.example to .env and fill in API values to enable this page.")
    else:
        structured = structure_requirements(st.session_state.requirements)
        risks = analyze_risks(structured)
        coverage_items = identify_coverage_items(structured, risks)
...
if page == "6. Export":
    st.subheader("Export")
    st.info("Exporter scaffold is prepared in src/exporter.py. CSV/JSON/Excel export will be implemented here.")
